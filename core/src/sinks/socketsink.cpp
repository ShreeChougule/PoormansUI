...
SocketSink::SocketSink() {
  ...
  if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
    // MSDN: An application can call the WSAGetLastError() function to determine
    ...
    // called in the case of a WSAStartup failure
...
SocketSink::~SocketSink() { closeSocket(); }

void SocketSink::resetSocket() {
  closeSocket();
  sock_cli = socket(AF_INET, SOCK_STREAM, 0);
  if (sock_cli < 0) {
...
bool SocketSink::Connect(const char* ip, const unsigned int port) {
  /// Define sockaddr_in
...
bool SocketSink::connected() { return isConnected; };

int SocketSink::RecieveData(char data[]) {
  int len = ::recvfrom(sock_cli, data, 300, 0, NULL, 0);
  if (0 < len) printf("receiveVehicleData: %s\n", data);
  ...
  //    }
  //    /*When the user enters the information, he begins to process the
  //    information and send it.*/ if(FD_ISSET(0, &rfds)){
...
int SocketSink::SendData(const char* data) {
  char buf[1024];
  strcpy(buf, data);
